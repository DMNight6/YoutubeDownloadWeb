...
const CardRender = ({product, download}) => { // I dunno what name I want to give so I'll call it product although it's data from backend
    ...
    let audio = new Audio(product.videoURL)
    const handleAudioButton = () => {
        AudioState ? audio.pause() : audio.play();
        setAudioState(AudioState ? false : true);
    }
        ...
        <Card sx={{ display: 'flex'}}>
            <Box sx={{ display: 'flex', flexDirection: 'column'}}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', pl: 1, pb: 1}}>
                    <IconButton aria-label='play/pause' onClick={e => handleAudioButton()}>
            ...
            <CardMedia sx={{ width: 151 }} image={product.thumbnail} />
        </Card>
    )
}
